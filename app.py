                    ...
                    try:
                        ...
                        for target_id in target_ids:
                            if target_id.isdigit() and emote_id_to_send.isdigit():
                                ...
                                # Send the emote 7 times, then wait 15 seconds (interruptible)
                                ...
                                # Wait 15 seconds after sending all 7 packets (interruptible by new commands)
                                logging.info(f"[ESPERANDO] Esperando 15 segundos después de enviar los 7 paquetes (interrumpible)...")
                                wait_time = 15.0
                                check_interval = 0.5  # Check every 0.5 seconds
                                elapsed = 0.0
                                
                                # Make socket non-blocking temporarily to check for new data
                                import select
                                original_timeout = socket_client.gettimeout()
                                socket_client.settimeout(0.1)
                                while elapsed < wait_time:
                                    try:
                                        # Check if there's new data available (non-blocking)
                                        ready, _, _ = select.select([socket_client], [], [], 0.1)
                                        if ready:
                                            # New data available, interrupt the wait
                                            logging.info(f"[INTERRUPCIÓN] Nuevo comando detectado durante la espera. Interrumpiendo espera de emote #{emote_choice}.")
                                            break
                                    except:
                                        pass
                                ...
                                socket_client.settimeout(original_timeout)
                                
                                if elapsed >= wait_time:
                                    logging.info(f"[OK] Emote #{emote_choice} completado (7 envíos + 15 segundos) para {target_id}")
                                else:
                                    logging.info(f"[OK] Emote #{emote_choice} interrumpido después de {elapsed:.1f} segundos (nuevo comando recibido)")
                            ...
                            if target_id.isdigit() and emote_id.isdigit():
                                ...
                                while elapsed < wait_time:
                                    try:
                                        ...
                                        if ready:
                                            # New data available, interrupt the wait
                                            logging.info(f"[INTERRUPCIÓN] Nuevo comando detectado durante la espera. Interrumpiendo espera de emote {emote_id}.")
                                            break
                                    except:
                                        pass
                                ...
                                if elapsed >= wait_time:
                                    ...
                                    logging.info(f"[OK] Emote {emote_id} interrumpido después de {elapsed:.1f} segundos (nuevo comando recibido)")
        ...
        for target_id in target_ids:
            if target_id.isdigit() and emote_id_to_send.isdigit():
                ...
                if command_type == '/ev' or category == "evolutivas":
                    ...
                    # Wait 15 seconds after sending all 7 packets (interruptible by new commands)
                    logging.info(f"[PANEL] ESPERANDO - Esperando 15 segundos después de enviar los 7 paquetes (interrumpible)...")
                    wait_time = 15.0
                    check_interval = 0.5  # Check every 0.5 seconds
                    elapsed = 0.0
                    ...
                    while elapsed < wait_time:
                        try:
                            ...
                            if ready:
                                # New data available, interrupt the wait
                                logging.info(f"[PANEL] INTERRUPCIÓN - Nuevo comando detectado durante la espera. Interrumpiendo espera de emote #{emote_choice}.")
                                break
                        except:
                            pass
                    ...
                    if elapsed >= wait_time:
                        ...
                        logging.info(f"[PANEL] OK - Emote #{emote_choice} interrumpido después de {elapsed:.1f} segundos (nuevo comando recibido)")
...
class PanelHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _send_cors_headers(self):
        """Envía headers CORS en todas las respuestas"""
        origin = self.headers.get('Origin', '*')
        # Permitir específicamente GitHub Pages y localhost para desarrollo
        allowed_origins = [
            'https://david32094.github.io',
            'http://localhost:3000',
            'http://localhost:8080',
            'http://127.0.0.1:3000',
            'http://127.0.0.1:8080'
        ...
        # Si el origen está en la lista permitida, usarlo; si no, usar '*'
        ...
        self.send_header('Access-Control-Allow-Headers', 'Content-Type, Authorization, ngrok-skip-browser-warning, X-Requested-With, Accept')
        self.send_header('Access-Control-Max-Age', '3600')
        self.send_header('Access-Control-Allow-Credentials', 'false')
    
    def do_OPTIONS(self):
        """Manejar preflight CORS - CRÍTICO para que funcione desde GitHub Pages"""
        origin = self.headers.get('Origin', 'Unknown')
        logging.info(f"[CORS] OPTIONS (preflight) recibido desde: {origin}")
        logging.info(f"[CORS] Path: {self.path}")
        logging.info(f"[CORS] Headers recibidos: {dict(self.headers)}")
        
        # Responder inmediatamente con headers CORS
        self.send_response(200)
        self._send_cors_headers()
        self.end_headers()
        
        logging.info(f"[CORS] OPTIONS response enviado correctamente")
    ...
    def do_POST(self):
        ...
        origin = self.headers.get('Origin', 'Unknown')
        logging.info(f"[CORS] POST request recibido desde: {origin}")
        logging.info(f"[CORS] Path: {self.path}")
        ...
        if self.path == '/api/send-command':
            try:
                ...
                # Enviar respuesta con headers CORS completos
                self.send_response(200)
                self.send_header('Content-Type', 'application/json')
                self._send_cors_headers()
                self.end_headers()
                self.wfile.write(json_module.dumps(result).encode('utf-8'))
                logging.info(f"[CORS] POST response enviado con headers CORS")
            ...
            # Para cualquier otra ruta, también enviar CORS headers
            self.send_response(404)
            self._send_cors_headers()
            self.end_headers()
